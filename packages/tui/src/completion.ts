import { editInput, type EditorKey, type EditorResult, type EditorState } from './editor.ts';
...
export function matchingCommands(state: CompletionState, commands: readonly CommandSuggestion[]): readonly CommandSuggestion[] {
  if (state.dismissedValue === state.value || state.cursor !== state.value.length || !/^\/[^\s/]*$/.test(state.value)) return [];
...
export function completeInput(
  ...
  if (matches.length && !key.ctrl && !key.meta && !key.shift) {
    if (key.upArrow || key.downArrow) {
      return { state: { ...state, selected: (selected + (key.upArrow ? -1 : 1) + matches.length) % matches.length } };
    }
    if (key.tab || key.return) {
      const command = matches[selected]!;
      const value = command.name + (command.arguments ? ' ' : '');
      return { state: { value, cursor: value.length, selected: 0, dismissedValue: value } };
    }
  }
