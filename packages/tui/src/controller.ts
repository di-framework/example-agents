...
export async function runChat(
  ...
  const perform = async (work: (context: CommandContext) => Promise<string | void> | string | void) => {
    ...
    try {
      ...
      terminal.write(controller.signal.aborted
  ...
  const respond = (message: string) => perform(async ({ signal }) => {
  ...
  try {
    ...
    while (!stopped) {
      ...
      if (!command) continue;
      switch (command) {
        case '/help': terminal.write(options.help); break;
        case '/clear':
          await perform(async () => {
        ...
        case '/paste':
          ...
          break;
        default: {
          if (!command.startsWith('/')) { await respond(line); break; }
          const name = command.split(/\s/, 1)[0];
          const handler = options.commands?.find((entry) => entry.name === name);
          if (handler) await perform((context) => handler.run(command.slice(handler.name.length).trim(), context));
