...
export function createReadlineTerminal({ input, output, interrupts }: ReadlineOptions): ChatTerminal {
  const interactive = Boolean(input.isTTY && output.isTTY);
  const readline = createInterface({ input, output, terminal: interactive, crlfDelay: Infinity });
  ...
  return {
    async readLine(prompt) {
      ...
      if (interactive) { readline.setPrompt(prompt); readline.prompt(); }
      const result = await lines.next();
      return closed || result.done ? null : result.value;
    },
    write(text, options) { output.write(`${options?.role === 'assistant' ? 'Agent: ' : ''}${text}\n\n`); },
    onInterrupt(handler) {
