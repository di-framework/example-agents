...
import { completeInput, matchingCommands, type CompletionState } from './completion.ts';
...
export function MessageView({ message }: { message: ChatMessage }) {
  const color = { user: 'green', assistant: 'yellow', info: 'gray', error: 'red' }[message.role];
  const label = message.role === 'user' ? 'You' : message.role === 'assistant' ? 'Agent' : undefined;
...
export function ChatView({ model }: { model: TerminalModel }) {
  ...
  const [editor, setEditor] = useState<CompletionState>({ value: '', cursor: 0, selected: 0 });
  ...
  return <Box flexDirection="column">
    ...
    {!snapshot.closed && !snapshot.inputEnded && <>
      ...
      <CommandPicker commands={matchingCommands(editor, snapshot.commands)} selected={editor.selected} />
    </>}
  </Box>;
}
