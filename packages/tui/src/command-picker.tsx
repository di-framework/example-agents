...
export function CommandPicker({ commands, selected }: {
  ...
  return <Box flexDirection="column" marginLeft={2}>
    {commands.slice(start, start + 6).map((command, offset) => {
      const active = start + offset === index;
      return <Text key={command.name} color={active ? 'cyan' : 'gray'} bold={active}>
        {active ? '❯ ' : '  '}{command.name}{command.arguments ? ` ${command.arguments}` : ''}
        {command.description ? `  ${command.description}` : ''}
      </Text>;
    })}
    <Text dimColor>↑/↓ select · Tab/Enter complete · Esc dismiss{commands.length > 6 ? ` · ${index + 1}/${commands.length}` : ''}</Text>
  </Box>;
}
