...
import { commandSuggestions, completeInput, matchingCommands, type CompletionState } from '../src/core.ts';
...
const state = (value: string): CompletionState => ({ value, cursor: value.length, selected: 0 });
...
test('slash opens the catalog; prefix filtering resets selection and ignores prose and arguments', () => {
  ...
  expect(matchingCommands(filtered, commands).map((command) => command.name)).toEqual(['/intake', '/issues']);
...
test('argument commands leave room for a path; editing and Escape dismissal remain usable', () => {
  ...
  expect(completeInput(state('/i'), '', { return: true, meta: true }, commands).state.value).toBe('/i\n');
});

test('draft mode offers only draft controls and exit commands; built-ins cannot be shadowed', () => {
  expect(commandSuggestions(commands, true).map(({ name }) => name)).toEqual(['/send', '/cancel', '/exit', '/quit']);
  const catalog = commandSuggestions([{ name: '/help', description: 'wrong' }, { name: '/test' }, { name: '/test' }]);
