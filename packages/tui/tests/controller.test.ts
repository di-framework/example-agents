...
import { createReadlineTerminal, createTerminalModel, runChat, type ChatSession } from '../src/core.ts';

const options = { title: 'Test agent', help: 'Test help' };
const session = (overrides: Partial<ChatSession> = {}): ChatSession => ({
  async chat(message) { return { content: `reply: ${message}` }; },
  clearHistory() {},
  ...overrides,
});
function deferred<T>() {
...
test('routes commands, preserves multiline content, and treats draft commands as text', async () => {
  ...
  let clears = 0;
  let closes = 0;
  for (const line of ['', '/help', '/paste', 'first', '/custom literal', '/send', '/paste', 'discard', '/cancel', '/custom args', '/clear', '/unknown', '  hello  ', '/exit', 'ignored']) model.submit(line);
  await runChat(session({
    async chat(message) { prompts.push(message); return { content: 'ok' }; },
    clearHistory() { clears++; },
    close() { closes++; },
  }), model.terminal, {
    ...options,
...
test('cancellation suppresses late replies and progress, then permits another turn', async () => {
  ...
  const task = runChat(session(), model.terminal, {
    ...options,
    commands: [{ name: '/slow', async run(_, context) {
...
test('provider and history errors recover, including a failed initial turn', async () => {
  ...
  await runChat(session({
    ...
    clearHistory() { throw new Error('history unavailable'); },
  }), model.terminal, { ...options, initialMessage: 'start' });
...
test('idle interrupt resolves the pending read and closes the session once', async () => {
  const model = createTerminalModel();
  let closed = 0;
  const task = runChat(session({ close() { closed++; } }), model.terminal, options);
  model.interrupt();
  await task;
  model.interrupt();
...
test('cleans up the session even when terminal setup or cleanup fails', async () => {
  for (const phase of ['setup', 'read', 'close']) {
    ...
    if (phase === 'setup') terminal.onInterrupt = () => { throw new Error(phase); };
    if (phase === 'read') terminal.readLine = async () => { throw new Error(phase); };
    if (phase === 'close') terminal.close = () => { throw new Error(phase); };
    await expect(runChat(session({ close() { closed++; } }), terminal, options)).rejects.toThrow(phase);
    expect(closed).toBe(1);
  }
});
...
test('readline queues piped input while busy, drains at EOF, and removes signal handlers', async () => {
  ...
  let transcript = '';
  output.on('data', (data) => { transcript += data; });
  const terminal = createReadlineTerminal({ input, output, interrupts });
  const started = deferred<void>();
  const finish = deferred<void>();
  const prompts: string[] = [];
  const task = runChat(session({ async chat(message) {
    prompts.push(message);
    if (message === 'first') { started.resolve(); await finish.promise; }
...
test('redirected output stays plain even with TTY input, and SIGINT releases a pending read', async () => {
  ...
  const terminal = createReadlineTerminal({ input, output, interrupts });
  let closed = 0;
  const task = runChat(session({ close() { closed++; } }), terminal, options);
