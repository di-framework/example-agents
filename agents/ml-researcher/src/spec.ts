...
export const modelSpecSchema = z
  ...
  .superRefine((spec, context) => {
    ...
    if (
      ...
      fail(
        "Embeddings require a compatible pretrained model and embedding output",
      );
    if (spec.task === "embedding" && spec.standardize)
      fail("Token IDs cannot be standardized");
    ...
    if (spec.task === "classification" && spec.outputEncoding !== "logits")
      fail(
        "Classification training requires logits, not a probability output graph",
      );
    if (
      spec.task === "regression" &&
      (spec.outputEncoding !== "values" || spec.outputSize !== 1)
    )
    ...
    if (spec.source.kind === "mlp") {
      ...
      const parameters = sizes
        .slice(1)
        .reduce((sum, size, i) => sum + (sizes[i]! + 1) * size, 0);
      if (parameters > 1_000_000)
        fail("MLP template is limited to one million parameters");
    }
  });
