...
export async function safePath(root: string, path: string): Promise<string> {
  ...
  for (;;) {
    try {
      if (!within(base, await realpath(ancestor))) throw new Error('Symlink escapes configured root');
      return candidate;
    } catch (error) {
...
export class FileArtifacts implements ArtifactStore {
  ...
  async read(path: string) { return readFile(await safePath(this.root, path), 'utf8'); }
}
export const json = (value: unknown) => JSON.stringify(value, null, 2) + '\n';
export const identifier = (value: string) => {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,79}$/.test(value)) throw new Error('Use a short alphanumeric identifier with hyphens or underscores');
  return value;
};
