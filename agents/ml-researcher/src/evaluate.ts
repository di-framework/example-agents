...
export function poolEmbedding(
  ...
  if (!Number.isFinite(norm) || norm === 0)
    throw new Error("Invalid zero/nonfinite embedding");
...
export async function evaluate(
  ...
  try {
    if (
      !session.inputs.includes(spec.inputName) ||
      !session.outputs.includes(spec.outputName)
    )
    ...
    const predict = async (row: DataRow, positive = false) => {
      ...
      if (session.inputs.some((input) => !feeds[input]))
        throw new Error(
          "Model needs additional inputs not supported by this template",
        );
    ...
    if (spec.task === "regression") {
      value =
        predictions.reduce(
          (sum, pred, i) => sum + (pred[0]! - rows[i]!.label!) ** 2,
          0,
        ) / rows.length;
