...
export class ResearchRun {
  ...
  static async open(options: RunOptions) {
    ...
    const store =
      options.artifacts?.(root) ?? new FileArtifacts(await realpath(root));
    ...
    if (options.resume)
      for (const target of ["ml", "framework"] as const) {
        if (
          manifest.worktrees[target]?.state !== "preparing" &&
          manifest.worktrees[target]
        )
  ...
  async objective(text: string) {
    ...
    if (this.manifest.objective && this.manifest.objective !== text)
      throw new Error(
        "This run already has an objective; refine it in chat or start a new run",
      );
  ...
  private async git(cwd: string, args: string[], signal?: AbortSignal) {
    return assertProcess(
      await this.execute({ command: "git", args, cwd, signal }),
    ).stdout.trim();
  }
  async prepare(target: Target, signal?: AbortSignal): Promise<Worktree> {
    if (
      this.manifest.worktrees[target] &&
      this.manifest.worktrees[target]?.state !== "preparing"
    )
  ...
  async validate(target: Target, signal?: AbortSignal) {
    ...
    if (
      (await common(entry.path)) !==
      (await common(this.manifest.repositories[target].path))
    )
    ...
    if (
      (await this.git(entry.path, ["rev-parse", "HEAD"], signal)) !==
      entry.commit
    )
      throw new Error(
        "Worktree HEAD changed; expected the recorded base commit",
      );
    return entry;
  }
      ...
      try {
        ...
        for (const dir of await readdir(skillsRoot, { withFileTypes: true })) {
          ...
          const path = await safePath(
            root,
            `.agents/skills/${dir.name}/SKILL.md`,
          );
          try {
            records.push({
              path,
