...
const raw = { name: 'fixture', task: 'classification', datasetPath: 'rows.jsonl', datasetRevision: 'v1', source: { kind: 'mlp', hidden: [4] }, inputSize: 2, outputSize: 1, outputEncoding: 'logits', learningRates: [0.01, 0.02], target: { metric: 'accuracy', value: 0.95 } };
...
test('selects using validation only, reports unmet test targets, and never overwrites a build', async () => {
  ...
  await f.run.store.write('rows.jsonl', Array.from({ length: 60 }, (_, i) => JSON.stringify({ id: String(i), features: [i, i % 4], label: i % 2 })).join('\n'));
  const measurements: { path: string; ids: string[] }[] = [];
  const builder = new ModelBuilder(f.run, {
    native: async (_run, request) => {
      if (request.action === 'inspect') return JSON.stringify({ inputs: ['input'], outputs: ['output'], weights: ['W'] });
    ...
    evaluate: async (request) => {
      measurements.push({ path: request.model, ids: request.rows.map((row) => row.id) });
      const index = measurements.length;
      return { metric: 'accuracy', value: [0.4, 0.9, 0.8, 0.7][index - 1]!, samples: request.rows.length, metrics: {} };
    },
  });
...
test('unsupported ONNX preflight and malformed datasets produce failed artifacts before training', async () => {
  const f = await fixture(); temporary.push(f.root);
  await f.run.store.write('rows.jsonl', Array.from({ length: 60 }, (_, i) => JSON.stringify({ id: String(i), features: [i, 0], label: i % 2 })).join('\n'));
  let trains = 0;
  const builder = new ModelBuilder(f.run, { native: async (_run, request) => {
    if (request.action === 'build') { await writeFile(String(request.path), 'unsupported'); return '{}'; }
...
test('cancelled training retains status and never evaluates the test set', async () => {
  ...
  const builder = new ModelBuilder(f.run, {
    native: async (_run, request, signal) => {
      ...
      if (request.action === 'inspect') return JSON.stringify({ inputs: ['input'], outputs: ['output'], weights: ['W'] });
      if (request.action === 'train') { controller.abort(); signal!.throwIfAborted(); }
      return '{}';
    },
    evaluate: async () => { evaluations++; return { metric: 'accuracy', value: 0.5, samples: 12, metrics: {} }; },
  });
