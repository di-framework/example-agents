...
test('normal and bare repositories create pinned worktrees, preserve source changes, and resume', async () => {
  ...
  const resumed = await ResearchRun.open({ runsDir: join(f.root, 'runs'), repositories: f.repositories, resume: f.run.manifest.id });
  ...
  await expect(ResearchRun.open({ runsDir: join(f.root, 'runs'), repositories: f.repositories, resume: f.run.manifest.id })).rejects.toThrow('branch');
});
...
test('untracked source instructions and skills remain discoverable, including missing references', async () => {
  ...
  await writeFile(join(f.ml, 'AGENTS.md'), 'Read `docs/missing.md` before claiming the roadmap complete.');
  await writeFile(join(f.ml, '.agents/skills/local/SKILL.md'), '---\nname: local\ndescription: fixture\n---\nLocal guidance');
  const guidance = await f.run.guidance('ml');
  expect(guidance.records.some((record) => record.content.includes('Local guidance') && record.provenance === 'source-local')).toBe(true);
...
test('process timeouts and aborts terminate descendants and bound output', async () => {
  const timeout = await runProcess({ command: process.execPath, args: ['-e', 'setInterval(() => {}, 1000)'], cwd: process.cwd(), timeoutMs: 40 });
  expect(timeout.timedOut).toBe(true);
  const controller = new AbortController();
  const task = runProcess({ command: process.execPath, args: ['-e', 'setInterval(() => {}, 1000)'], cwd: process.cwd(), signal: controller.signal });
  controller.abort();
  expect((await task).cancelled).toBe(true);
  const output = await runProcess({ command: process.execPath, args: ['-e', 'process.stdout.write("x".repeat(100000))'], cwd: process.cwd() });
...
test('resume recovers a worktree created just before an interrupted Git response', async () => {
  const f = await fixture(); temporary.push(f.root);
  const run = await ResearchRun.open({ runsDir: join(f.root, 'recovery'), repositories: f.repositories,
    runner: async (request) => {
      const result = await runProcess(request);
      if (request.args[0] === 'worktree' && request.args[1] === 'add') throw new Error('Interrupted after creation');
      return result;
    },
  });
  await expect(run.prepare('ml')).rejects.toThrow('Interrupted');
  expect(run.manifest.worktrees.ml?.state).toBe('preparing');
  const resumed = await ResearchRun.open({ runsDir: join(f.root, 'recovery'), repositories: f.repositories, resume: run.manifest.id });
...
test('timeout kills a descendant that ignores SIGTERM', async () => {
  ...
  const result = await runProcess({ command: process.execPath, args: ['-e', code, pidPath], cwd: f.root, timeoutMs: 500 });
