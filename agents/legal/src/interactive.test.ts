...
test("session memory retains exchanges without retaining stale case-data snapshots", async () => {
  ...
  const model = new FakeChatModel((prompt) =>
    ChatResponse.of(
      prompt.getUserMessage().text === "Begin intake"
        ? "Which state?"
        : "Recorded.",
    ),
  );
  ...
  try {
    ...
    expect(
      prompt?.messages.some((message) => message.text === "Begin intake"),
    ).toBe(true);
    expect(
      prompt?.messages.some((message) => message.text === "Which state?"),
    ).toBe(true);
    ...
    expect(JSON.stringify(memory.get("session"))).not.toContain(
      "case-data/README.md",
    );
    memory.clear("session");
    await legal.agent.chat("Start fresh");
    expect(
      model.calls[2]?.messages.some((message) => message.text === "Virginia"),
    ).toBe(false);
  } finally {
    await legal.close();
