...
if (import.meta.main) {
  ...
  if (values.help) {
    ...
    console.log(
      "Opens an interactive chat using your existing Codex ChatGPT subscription login.",
    );
    console.log(`Case workflow: ${CASE_WORKFLOW_SEQUENCE}`);
    console.log(
      "Type a stage command or a message, /help for commands, or /exit to finish.",
    );
    console.log(
    ...
    try {
      console.error(
        `Plugin: ${legal.plugin.name}; skills: ${legal.toolbox.skills.length}`,
      );
