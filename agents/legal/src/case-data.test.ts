...
test("case facts enter the user context once across a real Skill loop and refresh per request", async () => {
  ...
  try {
    ...
    await writeFile(file, "# Case\nState: Virginia (VA)\nCase label: UPDATED");
    await legal.agent.chat("Complete intake");
    expect(JSON.stringify(model.calls[2]?.messages)).toContain(
      "Case label: UPDATED",
    );
    expect(JSON.stringify(model.calls[2]?.messages)).not.toContain(
      "Case label: FIRST",
    );
  } finally {
    await legal.close();
...
test("missing or empty README leaves conversational intake available", async () => {
  ...
  try {
    ...
    for (const prompt of model.calls)
      expect(prompt.getUserMessages().map((message) => message.text)).toEqual([
        "Begin intake",
      ]);
  } finally {
    await legal.close();
...
test("oversized summaries and links outside the workspace fail before inference", async () => {
  ...
  try {
    await expect(legal.agent.chat("Begin intake")).rejects.toThrow(
      "exceeds 64 KiB",
    );
    ...
    await expect(readCaseData(directory)).rejects.toThrow(
