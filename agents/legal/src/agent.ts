...
export async function connectPluginMcp(
  ...
  if (
    ...
    throw new Error(
      `MCP ${name} must specify exactly one of command or serverUrl`,
    );
  const client = new Client({ name: "legal-agent", version: "0.1.0" });
  const transport = server.command
    ? new StdioClientTransport({
        command:
          server.command === "bun" ? process.execPath : expand(server.command),
...
export function loadLegalPlugin(workspace = process.cwd()): AgentPlugin {
  ...
  const directory = existsSync(preferred)
    ? preferred
    : resolve(workspace, ".agents/plugins/legal");
  ...
  const plugin = plugins.find((plugin) =>
    ["legal", "legal-plugin"].includes(plugin.name),
  );
  if (!plugin?.skillsDirectory)
    throw new Error(`Expected legal plugin skills under ${directory}`);
  return plugin;
}
  ...
  try {
    if (options.mcp !== false) {
      for (const [name, server] of Object.entries(
        ...
        try {
          sessions.push(
            await (options.connectMcp ?? connectPluginMcp)(
              name,
              server,
              plugin,
            ),
