...
import {
  type Graphql,
  LegalCatalog,
} from "../.agents/plugins/legal/mcp/catalog.ts";
...
test("pins a commit, pages and caches results, and preserves file contents", async () => {
  ...
  expect(
    requests
      .slice(1)
      .every((r) => r.variables.expression?.startsWith(`${commit}:`)),
  ).toBe(true);
  expect(
    requests.every(
      (r) =>
        !r.query.includes("mutation") && r.variables.owner === "ThomasMoreAI",
    ),
  ).toBe(true);
});
...
test("enforces a session request budget and reports missing objects", async () => {
  ...
  for (let index = 0; index < 99; index++)
    await catalog.list(`directory-${index}`);
...
`,
  ...
  try {
    ...
    try {
      const tool = session.tools.find((t) =>
        t.toolDefinition.name.endsWith("catalog_open"),
      );
