  ...
  const vectors = new CourtListenerVectors(
    ...
    },
    {
      send: async (command: any, options: any) => {
        ...
        if (command instanceof GetVectorsCommand)
          return { vectors: [{ key: "100:1", metadata }] };
...
test("DI S3VectorStore maps AWS distance, query vectors, metadata filtering and live coverage", async () => {
  ...
  expect(
    calls.find((c) => c.command instanceof GetObjectCommand)!.input.Key,
  ).toBe("imports/modernbert-768/status.json");
...
test("invalid arguments and incompatible embeddings fail before AWS search", async () => {
  ...
  await expect(
    embedQuery("test", undefined, async () => [1, 2]),
  ).rejects.toThrow("768");
  await expect(
    embedQuery("test", undefined, async () => Array(768).fill(NaN)),
...
test("oversized text is fetched only from its exact owned S3 key as text", async () => {
  ...
  await expect(
    bad.call("get_opinion_chunk", { opinion_id: 100, chunk_number: 1 }),
  ).rejects.toThrow("Unexpected chunk");
});
...
test("semantic search runs through the MCP protocol without a CourtListener API token", async () => {
  ...
  const server = createCourtListenerServer(
    new CourtListener("", fetch, vectors),
  );
