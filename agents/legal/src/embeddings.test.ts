...
test("masked pooling ignores padding and returns unit length vectors", () => {
  ...
  expect(() => poolAndNormalize(hidden, [1, 3, DIMENSIONS], [0, 0, 0])).toThrow(
    "empty",
  );
  expect(() => poolAndNormalize(hidden, [1, 2, DIMENSIONS], [1, 1, 0])).toThrow(
    "shape",
  );
  hidden[0] = NaN;
  expect(() => poolAndNormalize(hidden, [1, 3, DIMENSIONS], [1, 1, 0])).toThrow(
    "finite",
  );
});
...
test("unverified, stale, or damaged bundles fail before the ONNX runtime loads", async () => {
  ...
  try {
    await expect(CourtListenerEmbedder.load(directory)).rejects.toThrow(
      "missing",
    );
    const manifest = JSON.stringify({
      ...
      sha256: Object.fromEntries(
        ARTIFACTS.map((name) => [name, "a".repeat(64)]),
      ),
    });
...
test("embedding worker reuses a session, preserves literal text, and recovers after cancellation", async () => {
  ...
  try {
    ...
    await expect(
      client.embed("hang", AbortSignal.timeout(50)),
    ).rejects.toThrow();
    ...
    controller.abort();
    await expect(
      client.embed("never run", controller.signal),
    ).rejects.toThrow();
    expect(spawned).toBe(2);
  } finally {
    client.close();
...
test("queued embedding requests serialize and worker errors and timeouts recover", async () => {
  ...
  try {
    expect(
      await Promise.all([client.embed("one"), client.embed("two")]),
    ).toEqual([
      [1, 3],
      [2, 3],
    ]);
...
test("cancelling a queued request leaves the active request intact; close releases both", async () => {
  ...
  const controller = new AbortController();
  const queued = client
    .embed("queued", controller.signal)
    .catch((error) => error);
