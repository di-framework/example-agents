...
test("workspace agent instructions govern literal commands and reload in a new session", async () => {
  ...
  try {
    ...
    await writeFile(
      instructionsPath,
      "Custom workflow: use REVIEW-NOTE-A for /timeline.\n",
    );
    const firstModel = new FakeChatModel(() =>
      ChatResponse.of("First response"),
    );
    ...
    try {
      ...
      expect(firstModel.calls[0]?.getSystemMessage().text).toContain(
        "REVIEW-NOTE-A",
      );
    ...
    await writeFile(
      instructionsPath,
      "Custom workflow: use REVIEW-NOTE-B for /timeline.\n",
    );
    const secondModel = new FakeChatModel(() =>
      ChatResponse.of("Second response"),
    );
    ...
    try {
      ...
      expect(secondModel.calls[0]?.getSystemMessage().text).toContain(
        "REVIEW-NOTE-B",
      );
      expect(secondModel.calls[0]?.getSystemMessage().text).not.toContain(
        "REVIEW-NOTE-A",
      );
    } finally {
      await second.close();
...
test("a new agent session can read a previous stage artifact through the real workspace tools", async () => {
  ...
  try {
    ...
    const timelinePath = join(
      workspace,
      "legal-references/VA/case-work/timeline.md",
    );
    const issuesPath = join(
      workspace,
      "legal-references/VA/case-work/issues.md",
    );
    await writeFile(
      evidencePath,
      "SOURCE-E1: A meeting was reported on 2026-09-01.\n",
    );
    const timeline =
    ...
    const timelineModel = new FakeChatModel((prompt) => {
      ...
      switch (timelineCalls++) {
        ...
        case 1:
          ...
          return toolCallResponse([
            toolCall("evidence", "Read", { filePath: evidencePath }),
          ]);
        case 2:
      ...
      switch (issueCalls++) {
        case 0:
          ...
          expect(JSON.stringify(prompt.messages)).not.toContain(
            "Saved the provisional timeline.",
          );
          return toolCallResponse([
        ...
        case 1:
          ...
          return toolCallResponse([
            toolCall("prior-timeline", "Read", { filePath: timelinePath }),
          ]);
        case 2:
        ...
        default:
          ...
          return ChatResponse.of(
            "Saved provisional issues linked to timeline E1.",
          );
      }
    });
    ...
    expect(secondUi.output).toContain(
      "Saved provisional issues linked to timeline E1.",
    );
    ...
    expect(await readFile(timelinePath, "utf8")).toBe(timeline);
    expect(await readFile(join(workspace, "case-data/README.md"), "utf8")).toBe(
      caseData,
    );
  } finally {
    await rm(workspace, { recursive: true, force: true });
