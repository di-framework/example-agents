...
export const githubGraphql: Graphql = async <T>(
  ...
  if (!executable)
    throw new Error(
      "Install GitHub CLI and run gh auth login, or set GH_TOKEN",
    );
  ...
  try {
    ...
    if (code !== 0)
      throw new Error(
        "GitHub GraphQL request failed; check gh auth status and GitHub rate limits",
      );
...
function pathValue(value: string): string {
  if (
    ...
    throw new Error(
      "Use a repository-relative path without traversal or revision syntax",
    );
  return value;
}
function integer(
...
export class LegalCatalog {
  ...
  private files = new Map<
    string,
    { oid: string; text: string; byteSize: number }
  >();
  ...
  private async query<T>(
    ...
    return this.graphql<T>(
      query,
      { owner: OWNER, name: REPOSITORY, ...variables },
      signal,
    );
  }
  ...
  async list(path = "", offset = 0, limit = 40, signal?: AbortSignal) {
    ...
    if (!entries) {
      const object = await this.object(
        path,
        "... on Tree{entries{name type mode oid}}",
        signal,
      );
  ...
  async read(path: string, offset = 0, limit = 12_000, signal?: AbortSignal) {
    ...
    if (!file) {
      ...
      const metadata = await this.object(
        path,
    ...
    if (offset > file.text.length)
      throw new Error("offset is past the end of the file");
    ...
    return {
      ...
      nextOffset:
        offset + text.length < file.text.length ? offset + text.length : null,
      contentKind:
        "repository text; preserve skill frontmatter, licensing, and attribution",
    };
  }
}
