...
export class EmbeddingClient {
  ...
  constructor(private readonly createWorker = () => new Worker(new URL('./embedding-worker.ts', import.meta.url).href),
      ...
      return new Promise<number[]>((resolve, reject) => {
        if (!this.worker) {
          ...
          worker.onerror = event => {
            if (this.worker === worker) this.cancel?.(new Error(event.message || 'Embedding worker failed'));
          };
        }
        const worker = this.worker;
        const bounded = AbortSignal.any([AbortSignal.timeout(this.timeoutMs), ...(signal ? [signal] : [])]);
        const cleanup = () => {
        ...
        try { worker.postMessage({ text }); } catch (error) { fail(error as Error); }
      });
    });
