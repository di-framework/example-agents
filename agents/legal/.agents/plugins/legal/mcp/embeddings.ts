...
export const MODEL_DIRECTORY = resolve(
  import.meta.dir,
  "../../../../.cache/courtlistener-onnx",
);
...
export class CourtListenerEmbedder {
  ...
  static async load(
    ...
    if (options.verify !== false) {
      ...
      try {
        verified = JSON.parse(
          await readFile(join(directory, "verified.json"), "utf8"),
        );
      } catch {
        throw new Error(
          "CourtListener ONNX bundle has not passed parity verification. " +
            SETUP,
        );
      }
      if (
        verified.passed !== true ||
        verified.manifestSha256 !== sha256(manifestBytes)
      ) {
        throw new Error(
          "CourtListener ONNX parity verification is stale. " + SETUP,
        );
      }
    }
    ...
    for (const name of ARTIFACTS) {
      ...
      if (sha256(bytes) !== manifest.sha256[name])
        throw new Error(
          `CourtListener bundle checksum mismatch: ${name}. ${SETUP}`,
        );
  ...
  tokenize(text: string, prefix = QUERY_PREFIX): number[] {
    ...
    return ids.length <= MAX_LENGTH
      ? ids
      : [...ids.slice(0, MAX_LENGTH - 1), ids.at(-1)!];
  }
