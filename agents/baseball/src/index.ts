...
Default storage: agents/baseball/data/baseball.sqlite. Run bun run demo for a synthetic example.`);
      ...
      if (
        [values.report, values.export, values.photo, values.video].filter(
          (v) => v !== undefined,
        ).length > 1
      )
        throw new Error(
          "Choose one of --report, --export, --photo, or --video",
        );
      if (
        !values.video &&
        [values.start, values.duration, values.fps, values.output].some(
          (v) => v !== undefined,
        )
      )
        throw new Error(
          "--start, --duration, --fps, and --output require --video",
        );
