...
test('shared chat routes media commands with quoted paths, progress, and draft notices', async () => {
  ...
  const session: BaseballSession = {
    agent: { async chat(message) { calls.push(message); return { content: 'reply' }; } },
    clearHistory() { cleared++; },
    async readPhoto(path, signal) { calls.push(`photo:${path}`); mediaSignal = signal; return { draft: 'photo' }; },
    async watchVideo(path, options) {
      calls.push(`video:${path}`);
      // The UI only consumes coverage from the progress object.
      await options?.onProgress?.({ coverage: { analyzedThrough: 30, requestedEnd: 120 } } as Parameters<NonNullable<NonNullable<typeof options>['onProgress']>>[0]);
      return { draft: 'video' };
    },
  };
  const statuses: (string | null)[] = [];
  model.subscribe(() => { statuses.push(model.getSnapshot().status); });
  for (const line of ['/photo "score book.jpg"', "/video 'game clip.mp4'", '/photo', '/photo ""', '/clear', '/paste', 'first', 'second', '/send', '/exit']) model.submit(line);
  await runInteractive(session, model.terminal);
  expect(calls).toEqual(['photo:score book.jpg', 'video:game clip.mp4', 'first\nsecond']);
  expect(mediaSignal).toBeInstanceOf(AbortSignal);
  expect(cleared).toBe(1);
  expect(statuses).toContain('Analyzed through 30.0s / 120.0s');
  const text = model.getSnapshot().messages.map((entry) => entry.content).join('\n');
...
test('cancelled media results are suppressed and the next chat turn still runs', async () => {
  const model = createTerminalModel();
  for (const line of ['/photo image.jpg', '/video game.mp4', 'continue', '/exit']) model.submit(line);
  ...
  const text = model.getSnapshot().messages.map((entry) => entry.content).join('\n');
  expect(text).toContain('Cancelled. Check saved records before repeating an entry.');
