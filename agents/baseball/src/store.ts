...
export class BaseballStore {
  ...
  listTeams(): Team[] {
    return (
      this.db.query("SELECT payload FROM teams ORDER BY id").all() as Row[]
    ...
    return this.db
      .transaction(() => {
        ...
        if (existing) {
          ...
          throw new Error(
            "Team ID already exists; use a distinct ID for each team and season",
          );
        }
        this.db
          .query("INSERT INTO teams VALUES (?, ?)")
          .run(team.teamId, JSON.stringify(team));
        return team;
      })
      .immediate();
  }
  ...
  private team(teamId: string): Team {
    const row = this.db
      .query("SELECT payload FROM teams WHERE id = ?")
      .get(teamId) as Row | null;
    if (!row)
      throw new Error(`Unknown team ${teamId}; list or create teams first`);
  ...
  listGames(input: unknown): Game[] {
    ...
    return (
      this.db
        .query(
          "SELECT payload FROM games WHERE team_id = ? ORDER BY date, game_number, id",
        )
        .all(teamId) as Row[]
    ).map((row) => JSON.parse(row.payload));
  }
    ...
    return this.db
      .transaction(() => {
        const roster = new Set(
          this.roster({ teamId: data.teamId }).map((p) => p.playerId),
        );
        for (const line of data.lines) {
          if (!roster.has(line.playerId))
            throw new Error(`Unknown roster player ${line.playerId}`);
        }
        const oldRow = this.db
          .query("SELECT payload FROM games WHERE team_id = ? AND id = ?")
          .get(data.teamId, data.gameId) as Row | null;
        const old: Game | undefined = oldRow
          ? JSON.parse(oldRow.payload)
          : undefined;
        this.checkRevision(old, expectedRevision);
        if (old && !correctionReason)
          throw new Error("A correction reason is required to replace a game");
        const opponentKey = data.opponent
  ...
  voidGame(input: unknown): Game {
    const { teamId, gameId, expectedRevision, reason } =
      voidSchema.parse(input);
    return this.db
      .transaction(() => {
