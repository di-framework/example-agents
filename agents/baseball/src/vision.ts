...
const nullableCounts = (shape: Record<string, z.ZodType>) => z.strictObject(
...
export const scorebookSchema = z.strictObject({
  teamName: text, date: text, opponent: text, runsFor: number, runsAgainst: number,
...
export async function readImage(path: string) {
  ...
  try {
    ...
    if (info.size === 0 || info.size > MAX_IMAGE_BYTES) throw new Error('Image must be between 1 byte and 20 MiB');
    ...
    return { mimeType, data: bytes, name: basename(path), sha256: createHash('sha256').update(bytes).digest('hex') };
...
export async function readScorebook(model: ChatModel, path: string, signal?: AbortSignal) {
  ...
  catch { throw new Error('Vision model did not return valid JSON. No stats were saved.'); }
  const result = scorebookSchema.safeParse(parsed);
  if (!result.success) throw new Error('Vision model returned an invalid scorebook draft. No stats were saved.');
  return { source: `Photo ${image.name}; SHA-256 ${image.sha256}`,
    model: response.metadata.model ?? model.options?.model ?? 'configured vision model',
    reviewRequired: true as const, ...result.data };
}
