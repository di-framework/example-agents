...
export class CodexVisionModel implements ChatModel {
  readonly options;
  constructor(private config: { model?: string; executable?: string; timeoutMs?: number } = {}) {
    this.options = { model: config.model };
  }

  async call(prompt: Prompt): Promise<ChatResponse> {
    if (prompt.options?.toolCallbacks?.length) throw new Error('Vision extraction cannot execute application tools');
    const timeoutMs = this.config.timeoutMs ?? 120_000;
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1) throw new Error('Vision timeout must be positive');
    const signal = AbortSignal.any([AbortSignal.timeout(timeoutMs), ...(prompt.options?.signal ? [prompt.options.signal] : [])]);
    ...
    try {
      ...
      for (const message of prompt.messages) {
        ...
        for (const image of message.media) {
          if (!(image.data instanceof Uint8Array)) throw new Error('Vision images must be attached as bytes');
      ...
      child = handle;
      handle.stdin.write(prompt.messages.map((m) => `${m.messageType}: ${m.text ?? ''}`).join('\n\n'));
      handle.stdin.end();
      ...
      const boundedText = async (stream: ReadableStream<Uint8Array>) => {
        ...
        try {
          for (;;) {
            ...
            if (done) break;
            length += value.length;
            if (length > 2 * 1024 * 1024) { kill(); throw new Error('Vision response exceeded 2 MiB'); }
            chunks.push(value);
          }
      ...
      const [stdout, stderr, code] = await Promise.all([
        ...
        boundedText(child.stderr as ReadableStream<Uint8Array>), child.exited,
      ]);
      signal.throwIfAborted();
      ...
      return ChatResponse.of(stdout.trim(), { model: model ?? 'Codex CLI default' });
    } finally {
