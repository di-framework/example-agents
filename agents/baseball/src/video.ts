...
export async function observeFrames(
  ...
  if (!result.success)
    throw new Error("Video model returned an invalid observation");
  ...
  if (
    ...
    throw new Error(
      "Replay/commercial scoreboard cannot update live observations",
    );
  for (const event of draft.events) {
    if (
      ...
      event.end >= end ||
      event.evidenceFrames.some(
        (t) => !available.has(t) || t < event.start || t > event.end,
      )
    )
      throw new Error(
        "Video event must cite supplied frames within its interval",
      );
...
export function candidateCounts(
  ...
  for (const event of windows.flatMap((window) => window.events)) {
    const root = event.duplicateOf
      ? (roots.get(event.duplicateOf) ?? event.duplicateOf)
      : event.id;
...
export async function watchVideo(
  ...
  if (
    ...
    fps > 2 ||
    (duration !== "all" &&
      (!Number.isFinite(duration) || duration <= 0 || duration > 21_600))
  )
    throw new Error(
      "Use start >= 0, duration 0–21600 seconds (or all), and fps 0.5–2",
    );
  const video = await inspectVideo(path, signal);
  if (start >= video.duration)
    throw new Error("Start must be before the end of the video");
  const end =
    duration === "all"
      ? video.duration
      : Math.min(video.duration, start + duration);
  if (end - start > 21_600)
    throw new Error(
  ...
  if (start > 0 || end < video.duration)
    draft.warnings.push(
      "Only the requested portion of this recording was analyzed.",
    );
  ...
  for (let cursor = start; cursor < end; ) {
    ...
    if (current.size !== video.bytes || current.mtimeMs !== video.modified)
      throw new Error(
        "Video changed during analysis; use a completed recording",
      );
