...
function sum<T extends Record<string, number>>(rows: T[]): T | null {
  ...
  for (const row of rows.slice(1)) {
    for (const key of Object.keys(row) as (keyof T)[]) result[key] = (result[key]! + row[key]!) as T[keyof T];
  }
  return result;
}
...
export function battingStats(rows: Batting[]) {
  ...
  return { ...b, PA: b.AB + b.BB + b.HBP + b.SF + b.SH + b.CI, TB,
    AVG: ratio(b.H, b.AB), OBP, SLG, OPS: OBP === null || SLG === null ? null : OBP + SLG };
}

export function pitchingStats(rows: Pitching[], basis: number) {
  const p = sum(rows);
  return p && { ...p, IP: innings(p.outs), ERA: ratio(p.ER * basis * 3, p.outs),
...
export function seasonReport(team: Team, roster: Player[], games: Game[]) {
  ...
  const summaries = roster.map((player) => {
    const entries = activeGames.flatMap((game) => game.lines
      .filter((line) => line.playerId === player.playerId).map((line) => ({ game, line })));
    ...
    return { ...player, recordedGames: entries.length,
      coverage: { battingGames: batting.length, pitchingGames: pitching.length,
        fieldingGames: fielding.length, pitchCountGames: pitches.length },
      batting: battingStats(batting), pitching: pitchingStats(pitching, team.inningsPerGame),
      fielding: fieldingStats(fielding),
      pitchCount: pitches.length ? pitches.reduce((sum, { line }) => sum + line.pitchCount!, 0) : null,
      pitchLog: pitches.map(({ game, line }) => ({ date: game.date, gameId: game.gameId,
        opponent: game.opponent, pitchCount: line.pitchCount })),
    };
  });
  const allLines = activeGames.flatMap((game) => game.lines);
  return {
    team, eraInnings: team.inningsPerGame,
    coverageNote: 'Totals include recorded lines only. Missing categories and undefined rates are null; coverage is not proof of a complete scorebook.',
    record: {
    ...
    pitching: pitchingStats(allLines.flatMap((line) => line.pitching ? [line.pitching] : []), team.inningsPerGame),
