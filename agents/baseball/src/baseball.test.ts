...
const team = { teamId: 'owls-2026', name: 'Owls', season: '2026', division: 'Majors', inningsPerGame: 6 as const };
const player = { teamId: team.teamId, playerId: 'alex', name: 'Alex', number: '7', active: true, expectedRevision: null };
const query = { teamId: team.teamId, from: null, through: null };
const batting: Batting = { AB: 3, H: 2, doubles: 1, triples: 0, HR: 0, R: 1, RBI: 2,
  BB: 1, HBP: 0, SO: 1, SB: 1, CS: 0, SF: 0, SH: 0, CI: 0 };
const game: GameInput = { teamId: team.teamId, gameId: 'foxes-1', date: '2026-09-12', opponent: 'Foxes',
  gameNumber: 1, runsFor: 5, runsAgainst: 3, source: 'Synthetic test scorebook',
  expectedRevision: null, correctionReason: null,
  lines: [{ playerId: 'alex', batting,
    pitching: { outs: 7, H: 2, R: 1, ER: 1, BB: 1, HBP: 0, SO: 3 },
    fielding: { PO: 1, A: 2, E: 1 }, pitchCount: 38 }],
};
...
test('calculates rates from cumulative opportunities, not averages of game averages', () => {
  const s = setup();
  s.saveGame(game);
  s.saveGame({ ...game, gameId: 'foxes-2', date: '2026-09-13', runsFor: 1, runsAgainst: 2,
    lines: [{ ...game.lines[0], batting: { ...batting, AB: 1, H: 0, doubles: 0, RBI: 0, BB: 0 },
      pitching: null, fielding: null, pitchCount: null }] });
  const r = s.report(query);
  expect(r.record).toEqual({ games: 2, wins: 1, losses: 1, ties: 0, runsFor: 6, runsAgainst: 5 });
  ...
  expect(r.players[0]?.coverage).toEqual({ battingGames: 2, pitchingGames: 1, fieldingGames: 1, pitchCountGames: 1 });
});

test('handles walks, sacrifices, interference, and zero-denominator rates', () => {
  const b = { ...batting, AB: 0, H: 0, doubles: 0, SO: 0, BB: 1, HBP: 1, SF: 1, SH: 1, CI: 1 };
  expect(battingStats([b])).toMatchObject({ PA: 5, TB: 0, AVG: null, OBP: 2 / 3, SLG: null, OPS: null });
  expect(battingStats([])).toBeNull();
  expect(pitchingStats([{ outs: 0, ER: 1, R: 1, H: 1, BB: 1, HBP: 0, SO: 0 }], 6))
...
test('records pitch counts independently and preserves missing categories', () => {
  ...
  s.saveGame({ ...game, lines: [{ playerId: 'alex', batting: null, pitching: null, fielding: null, pitchCount: 24 }] });
  const r = s.report(query);
  expect(r.batting).toBeNull();
  expect(r.players[0]).toMatchObject({ pitchCount: 24, batting: null, pitching: null });
  expect(r.players[1]).toMatchObject({ recordedGames: 0, pitchCount: null, batting: null });
  expect(r.players[0]?.pitchLog).toEqual([{ date: game.date, gameId: game.gameId, opponent: 'Foxes', pitchCount: 24 }]);
});

test('replaces whole games once, preserves history, voids, and restores', () => {
  const s = setup();
  s.saveGame(game);
  const corrected = { ...game, runsFor: 2, expectedRevision: 1, correctionReason: 'Scorebook correction' };
  expect(s.saveGame(corrected).revision).toBe(2);
  expect(s.report(query).record).toMatchObject({ games: 1, losses: 1, runsFor: 2 });
  ...
  s.saveGame({ ...corrected, expectedRevision: 3, correctionReason: 'Scorer confirmed original matchup' });
...
test('rejects duplicate matches but supports doubleheaders and other seasons', () => {
  ...
  s.saveGame(game);
  expect(() => s.saveGame({ ...game, gameId: 'oops', opponent: ' FOXES ' })).toThrow('already recorded');
...
test('rejects bad scorebooks atomically and keeps existing revision intact', () => {
  const s = setup();
  const invalid = [
    { date: '2026-02-30' }, { runsFor: -1 }, { unknown: true },
...
test('persists across connections and rejects stale writers', () => {
  ...
  first.saveGame({ ...game, runsFor: 6, expectedRevision: 1, correctionReason: 'Added missed run' });
  expect(() => second.saveGame({ ...game, expectedRevision: 1, correctionReason: 'Stale edit' })).toThrow('Revision conflict');
...
test('keeps retired player history and filters dates inclusively', () => {
  const s = setup();
  s.saveGame(game);
  s.savePlayer({ ...player, name: 'Alex J.', active: false, expectedRevision: 1 });
  expect(s.report({ ...query, from: game.date, through: game.date }).players[0])
    .toMatchObject({ name: 'Alex J.', active: false, recordedGames: 1 });
  expect(s.report({ ...query, from: '2026-09-13' }).record.games).toBe(0);
  expect(() => s.report({ ...query, from: '2026-09-13', through: game.date })).toThrow('Start date');
});
...
test('real DI Framework tool loop saves a game and reads calculated season stats', async () => {
  ...
  try {
    ...
    const response = await baseball.agent.chat('Record these confirmed postgame stats and show the season report.');
