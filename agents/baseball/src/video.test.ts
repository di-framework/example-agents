...
async function fixture(duration = 2) {
  ...
  await runMedia(['ffmpeg', '-v', 'error', '-f', 'lavfi', '-i', `testsrc2=size=320x180:rate=10:duration=${duration}`,
    '-c:v', 'mpeg4', path]);
  return path;
}
const frame = (time: number) => ({ time, mimeType: 'image/jpeg', data: Buffer.from([255, 216, 255, 1]) });
const scoreboard = { awayTeam: 'HOU', homeTeam: 'ATL', awayRuns: 1, homeRuns: 2, inning: 4,
  half: 'bottom' as const, outs: 1, balls: 0, strikes: 0, first: false, second: false, third: false };
function observation(): VideoObservation {
  return { scoreboard, scoreboardFrame: 2, broadcast: 'live', warnings: [], events: [{
    start: 0, end: 2, evidenceFrames: [0, 2], kind: 'home_run', presentation: 'live',
    batter: 'Olson', pitcher: 'Valdez', team: 'ATL', inning: 4, half: 'bottom', runsScored: 1,
    confidence: 'high', evidence: 'Ball clears the wall; score increases by one.', uncertainty: null, duplicateOf: null,
  }] };
}
const inference = (result: unknown) => new FakeChatModel(() => ChatResponse.of(JSON.stringify(result)));
...
test('video sends ordered images and timestamps, no tools, and bounded prior context', async () => {
  const model = new FakeChatModel((prompt) => {
    ...
    return ChatResponse.of(JSON.stringify(observation()), { model: 'test-vision' });
  });
  const result = await observeFrames(model, [frame(0), frame(1), frame(2)], 0, 3);
...
test('invalid timestamps, references, replay scoreboards, and malformed responses fail closed', async () => {
  ...
  const invalid: unknown[] = [
    { ...original, scoreboardFrame: 7 }, { ...original, scoreboardFrame: null },
    { ...original, broadcast: 'replay' },
    { ...original, scoreboard: { ...scoreboard, outs: 9 } },
    ...[
      { start: -1 }, { start: 2, end: 1 }, { end: 3 }, { evidenceFrames: [0, 1.5] },
      { duplicateOf: 'invented-event' }, { evidenceFrames: [] },
    ].map((change) => ({ ...original, events: [{ ...original.events[0], ...change }] })),
    { events: [] },
  ];
  for (const result of invalid) await expect(observeFrames(inference(result), [frame(0), frame(1), frame(2)], 0, 3)).rejects.toThrow();
  await expect(observeFrames(new FakeChatModel(() => ChatResponse.of('not JSON')), [frame(0)], 0, 1)).rejects.toThrow('valid JSON');
});

test('overlapping evidence is linked and replays/ambiguity never increment candidate counts', async () => {
  const result = await observeFrames(inference(observation()), [frame(0), frame(1), frame(2)], 0, 3);
  const original = result.events[0]!;
  const repeated = { ...observation(), events: [{ ...observation().events[0]!, start: 1, evidenceFrames: [1, 2] }] };
  ...
  const extras: VideoEvent[] = [
    { ...original, id: 'replay', presentation: 'replay', duplicateOf: original.id },
  ...
  expect(candidateCounts([result, overlap, { ...result, events: extras }])).toEqual({ home_run: 1 });
...
mediaTest('actual decoder uses source PTS at a nonzero seek, carries bytes, and bounds windows', async () => {
...
mediaTest('multiwindow video retains overlap, checkpoints coverage, and does not write stats', async () => {
  ...
  const model = new FakeChatModel((prompt) => {
    ...
    return ChatResponse.of(JSON.stringify({ ...observation(), scoreboardFrame: times.at(-1),
      events: [{ ...observation().events[0], start: 18, end: 19, evidenceFrames: [18, 19],
  ...
  const baseball = createBaseballAgent(chat, { visionModel: model, databasePath: ':memory:' });
  const checkpoints: number[] = [];
  try {
    const draft = await baseball.watchVideo(path, { duration: 'all', onProgress: (draft) => { checkpoints.push(draft.coverage.analyzedThrough); } });
...
mediaTest('bad sources/options fail before inference, and cancellation stops between windows', async () => {
  const path = await fixture(22);
  const model = inference({ scoreboard: null, scoreboardFrame: null, broadcast: 'unknown', events: [], warnings: [] });
  ...
  await expect(watchVideo(model, path, { duration: 'all', signal: controller.signal,
    onProgress: (draft) => { checkpoints.push(draft.status); controller.abort(); },
  })).rejects.toThrow();
...
mediaTest('decoder timeout terminates a stalled process', async () => {
  ...
  await expect(runMedia([process.execPath, '-e', 'setInterval(() => {}, 1000)'], undefined, 100)).rejects.toThrow();
...
test('video CLI rejects output aliasing its input, preserves source, and rejects unrelated options', async () => {
  ...
  const run = async (args: string[]) => {
    const child = Bun.spawn([process.execPath, join(import.meta.dir, 'index.ts'), ...args], { stdout: 'pipe', stderr: 'pipe' });
    return { error: await new Response(child.stderr).text(), code: await child.exited };
  };
  expect(await run(['--video', path, '--output', path])).toEqual({ code: 1, error: 'Output must not overwrite the source video\n' });
